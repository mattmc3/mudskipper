...
function assert.tables_equal(t1, t2, desc)
	...
	for i = 1, #t1 do
		if t1[i] ~= t2[i] then
			print(
				string.format(
					"FAIL: %s (tables differ at index %d: %s ~= %s)",
					desc,
					i,
					tostring(t1[i]),
					tostring(t2[i])
...
assert.error(function()
	random({ "1", "2", "3", "4" })
end, "random: errors on too many arguments")
assert.error(function()
	random({ "foo" })
end, "random: errors on non-numeric argument")
...
assert.error(function()
	math_command({ "fac", -1 })
end, "math_command: fac errors on negative input")
assert.error(function()
	math_command({ "ncr", 5, 6 })
end, "math_command: ncr errors when r > n")
assert.error(function()
	math_command({ "npr", 5, 6 })
end, "math_command: npr errors when r > n")
